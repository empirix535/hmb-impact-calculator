...
export function runBia(inputs: BiaInputs): BiaResult {
  ...
  const shift = computeShift(inputs.marketShares0, inputs.targetHIud, deltasN);
  const ms0 = inputs.marketShares0;
  const ms1 = shift.marketShares1;
  ...
  const breakdown: ArmBreakdown[] = armKeys.map((arm) => {
    ...
    let status: ArmBreakdown["status"] = "—";
    if (arm !== "hIud") {
      if (shift.clampedArms.includes(arm as AltArm)) status = "Clamped to 0%";
      else if (shift.reallocated && dM < 0) status = "Re-allocated";
    }
    return {
      arm,
      ms0: m0,
